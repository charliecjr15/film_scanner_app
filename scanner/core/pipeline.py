...
from scanner.core.image_io import read_image_with_profile, resize_to_max_edge
...
from scanner.core.frame_detector import detect_film_frame, estimate_content_mask
...
def process_image(job: ImageJob, preview: bool = False) -> np.ndarray:
    image, _meta = read_image_with_profile(job.source_path)
    ...
    if job.film_type == "color_negative":
        image = invert_color_negative(
            image,
            border_hint=not job.include_border,
